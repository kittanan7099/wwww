...
class Modules extends \Kotchasan\KBase
{
    ...
    {
        return $this->list($className, 'controller');
    }
    /**
    ...
    {
        return $this->list($className, 'model');
    }
    ...
    {
        return $this->list($className, 'view');
    }
    ...
    function list($className, $type) {
        $result = array();
        $file = strtolower($className);
        foreach ($this->modules as $module) {
            if (is_file($this->dir.$module.'/'.$type.'s/'.$file.'.php')) {
                require_once $this->dir.$module.'/'.$type.'s/'.$file.'.php';
                $result[] = '\\'.ucfirst($module).'\\'.$className.'\\'.ucfirst($type);
            }
        }
        return $result;
    }
